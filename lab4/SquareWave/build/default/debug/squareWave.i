...
mainLoop:
    ...
    MOVLW 0b00000100 ; turn on ((PORTC) and 0FFh), 2, a and all others off
    MOVWF LATC

squareWave:
    BSF ((LATC) and 0FFh), 0, a ; turn on ((LATC) and 0FFh), 0, a 1 cycle
    NOP ; wait 2 cycles
    NOP
    BCF ((LATC) and 0FFh), 2, a ; ((LATC) and 0FFh), 2, a to 0
    NOP ; wait 2 cycles
    NOP
    BSF ((LATC) and 0FFh), 1, a ; ((LATC) and 0FFh), 1, a to 1
    NOP ; wait 2 cycles
    NOP
    BCF ((LATC) and 0FFh), 0, a ; ((LATC) and 0FFh), 0, a to 0
    NOP ; wait 2 cycles
    NOP
    BSF ((LATC) and 0FFh), 2, a ; ((LATC) and 0FFh), 2, a to 1
    NOP ; wait 2 cycles
    NOP
    BCF ((LATC) and 0FFh), 1, a ; ((LATC) and 0FFh), 1, a to 1
