...
main: ; program starts here
    BANKSEL TRISF
    bcf ((TRISF) and 0FFh), 3, a ;make led bit output

    BANKSEL TRISB
    bsf ((TRISB) and 0FFh), 4, a ;make button bit input

    BANKSEL ANSELF
    bcf ((ANSELF) and 0FFh), 3, b ;make led bit digital

    BANKSEL ANSELB
    bcf ((ANSELB) and 0FFh), 4, b ;make button bit digital

    BANKSEL WPUB
    bsf ((WPUB) and 0FFh), 4, b ;enable weak pullup for button

    bsf ((TRISB) and 0FFh), 4, a
    bcf ((TRISF) and 0FFh), 3, a

loop:
    btfss ((PORTB) and 0FFh), 4, a ;register b bit 4 ;is button pressed?
    goto ledON ;yes - go to ledON

    bsf ((PORTF) and 0FFh), 3, a ;register f bit 3 ;no - turn off led
    goto loop ;loop again

ledON:
    bcf ((PORTF) and 0FFh), 3, a ;register f bit 3 ;turn on led
    goto loop ;loop again
end resetVec
